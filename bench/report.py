...
def main():
    ...
    if evs:
        ...
        for name, r in runs.items():  # runs trained with the rural set report the same held-out tiles themselves
            if "naip_test" in r and r["naip_test"]["n_tiles"] == ev["n_tiles"]:
                bh = r["naip_test"]["by_height"]
                big = [v for k, v in bh.items() if k in ("20-40m", "40-infm")]
                t20 = None
                if big:  # pool the >20 m bands, weighting by pixel fraction
                    w = np.array([v["n_px"] for v in big], float); t20 = dict(rmse=float(np.sqrt(np.sum(w * np.array([v["rmse"] ** 2 for v in big])) / w.sum())),
