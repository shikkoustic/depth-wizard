...
nm = glob.glob("/kaggle/input/**/naip_meta.json", recursive=True)[0]; ND = os.path.dirname(nm); NM = json.load(open(nm))
sp = np.array(NM["split"]); ite = np.flatnonzero(sp == "test")
X = np.load(f"{ND}/naip_rgb.npy", mmap_mode="r")[ite]; G = np.load(f"{ND}/naip_agl.npy", mmap_mode="r")[ite].astype(np.float32)
print("test tiles", len(ite), flush=True)
...
R = dict(n_tiles=len(ite), zero=stats(np.zeros_like(G), G), models={})
for ck in sorted(glob.glob("/kaggle/input/**/best.pt", recursive=True)):
    ...
    bands = {}
    for lo, hi in [(0, 2), (2, 5), (5, 10), (10, 20), (20, 1e4)]:
        mk = (G >= lo) & (G < hi)
        if mk.sum() > 100:
            bands[f"{lo}-{hi if hi < 1e4 else 'inf'}m"] = stats(P, G, mk)
    R["models"][name] = dict(checkpoint=ck, variant=variant, in_size=IN, all=stats(P, G), by_height=bands)
    print(name, json.dumps(R["models"][name]["all"]), flush=True)
